...
public class MTCNN {
    ...
    public Vector<Box> detectFaces(Bitmap bitmap, int minFaceSize) {
        ...
        long a= System.currentTimeMillis()-t_start;
        if(min>a) {
            min = a;
        }else if(max<a){
            max=a;
        ...
        total+= System.currentTimeMillis()-t_start;
        tno++;
        avg=total/tno;
        Log.i("testing",avg+"");
        Log.i("testing","min:-"+min+"  max:-"+max);
        //Log.i("yo","[*]Mtcnn Detection Time:"+(System.currentTimeMillis()-t_start));
        comp=1;
